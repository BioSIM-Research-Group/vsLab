...
if {$runs==1} {
       ...
       if {$erro!=0} {VsLab::debug text "Error : $erroMsg"}
		...
		set saveFile [open "$gridName.gpf" w]
			for {set a 1} {$a<=[expr $mem-1]} {incr a} {
				if {[llength [lindex [array get Input $a] 1]]!=2} {puts $saveFile [lindex [array get Input $a] 1]
				} else {puts $saveFile "[format %-35s [lindex [lindex [array get Input $a] 1] 0]]  [lindex [lindex [array get Input $a] 1] 1]"}
			}
	...
	## create autodock input file
        ...
        set erro 0
        set erro [catch {exec $autoDockToolsPath/bin/python $autoDockToolsPath/MGLToolsPckgs/AutoDockTools/Utilities24/prepare_dpf4.py -l $ligandOutput -r $proteinOutput -o "$dockName.dpf"} erroMsg ]
