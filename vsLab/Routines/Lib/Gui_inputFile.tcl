...
########## Frame 1
	...
	grid [ttk::frame $input.f1  ] -in $input -row 0 -column 0 -sticky news
...
########## Frame 2
	grid [ttk::labelframe $input.f2 -text "Ligands"] -in $input -padx 10 -row 1 -column 0 -pady 10 -sticky news 
...
########## Frame 3
        ...
        ## Label
	grid [ttk::checkbutton $input.f3.cb1 -variable VsLab::lCharge1 -text "Check the hydrogens in the ligand strutcure." ] -in $input.f3 -row 1 -column 0 -padx 18 -pady 5 -sticky w
	grid [ttk::checkbutton $input.f3.cb -variable VsLab::lCharge0 -text "Keep the original charges of the ligands." ] -in $input.f3 -row 0 -column 0 -padx 18 -pady 5 -sticky w
...
proc VsLab::create_inputFile {} {
	...
	set fileName [tk_getSaveFile -initialfile $fileName  -defaultextension ".vsLab" -filetypes $types]
		...
		set VsLab::progressMessage "Creating input Files ..."
		## Create the new directory
			...
			## Inactivate Run
			 $VsLab::topGui.nb1.f2.fb.bRun configure -state disable
        		...
        		#### Progress bar update maximum
			$VsLab::topGui.f3.pb configure -maximum [expr (2*[llength $VsLab::arrayLigandPath] +2)]
			...
			## options of the ligand chareg and hydrogen
			if {$VsLab::lCharge0==0 && $VsLab::lCharge1==0} {set VsLab::lCharge 0}
			if {$VsLab::lCharge0==1 && $VsLab::lCharge1==0} {set VsLab::lCharge 1}
			if {$VsLab::lCharge0==0 && $VsLab::lCharge1==1} {set VsLab::lCharge 2}
			if {$VsLab::lCharge0==1 && $VsLab::lCharge1==1} {set VsLab::lCharge 3}
                        ...
                        if {[outputdb eval {select count(*) from sqlite_master where name="input"}]!=1} {
                                outputdb eval "CREATE TABLE if not exists input (Output, Protein, Ligand, lCharge, grid_size, grid_center, grid_selection, grid_spacing, gridSample, ga_set, ga_run, ga_num_evals, ga_num_generations, ga_pop_size, dockSample)"
                                ...
                                foreach a $VsLab::arrayLigandPath {
                                        if {$a!=""} {
                                                ...
                                                outputdb eval "INSERT INTO input (Output, Protein, Ligand, lCharge, grid_size, grid_center, grid_selection, grid_spacing, gridSample, ga_set, ga_run, ga_num_evals, ga_num_generations, ga_pop_size, dockSample) VALUES (:directory, :protein, :a, :VsLab::lCharge, :VsLab::grid_size, :VsLab::grid_center, :VsLab::grid_selection, :VsLab::grid_spacing, :VsLab::gridSample, :VsLab::ga_set, :VsLab::ga_run, :VsLab::ga_num_evals, :VsLab::ga_num_generations, :VsLab::ga_pop_size, :VsLab::dockSample)"
						$VsLab::topGui.f3.pb step 1;update
                                        }
                                }
			...
			set VsLab::progressMessage "Creating [file rootname [file tail $ProteinFile] ].pdbqt .";update
			...
			foreach a $VsLab::arrayLigandPath {
				...
				set VsLab::progressMessage "Creating [file rootname [file tail $a]].pdbqt ."; update
				...
				} elseif {$VsLab::lCharge==2} {
                                        set erro [catch {exec $autoDockToolsPath/bin/python $autoDockToolsPath/MGLToolsPckgs/AutoDockTools/Utilities24/prepare_ligand4.py -A hydrogens -l $ligand -o $ligandOutput} erroMsg ]

				} elseif {$VsLab::lCharge==3} {
                                        set erro [catch {exec $autoDockToolsPath/bin/python $autoDockToolsPath/MGLToolsPckgs/AutoDockTools/Utilities24/prepare_ligand4.py -A hydrogens -C -l $ligand -o $ligandOutput} erroMsg ]
                                }
